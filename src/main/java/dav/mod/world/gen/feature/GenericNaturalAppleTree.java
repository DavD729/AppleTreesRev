...
public class GenericNaturalAppleTree extends Feature<DefaultFeatureConfig>{
	
	private final TreeFeatureConfig TreeConfig;
	
	public GenericNaturalAppleTree(Codec<DefaultFeatureConfig> configCodec, TreeFeatureConfig BaseConfig) {
		super(configCodec);
		this.TreeConfig = BaseConfig;
	}
	...
	public boolean generate(StructureWorldAccess WorldIn, ChunkGenerator chunkGenerator, Random random, BlockPos Pos, DefaultFeatureConfig config) {
		...
		boolean bl = this.generate(WorldIn, random, Pos, set, set2, blockBox, TreeConfig);
		if (blockBox.minX <= blockBox.maxX && bl && !set.isEmpty()) {
			...
			if (!TreeConfig.decorators.isEmpty()) {
				TreeConfig.decorators.forEach((decorator) -> {
					decorator.generate(WorldIn, random, list, list2, set3, blockBox);
				});
			}
			
			AppleDecorator AppleBase = new AppleDecorator(0, true); //Default Decorator of THIS TREE
			AppleBase.generate(WorldIn, random, list, list2, set3, blockBox);
			
			VoxelSet voxelSet = this.placeLogsAndLeaves(WorldIn, blockBox, set, set3);
			Structure.updateCorner(WorldIn, 3, voxelSet, blockBox.minX, blockBox.minY, blockBox.minZ);
			return true;
		} else {
			return false;
		}
			...
			if (!isDirtOrGrass(world, blockPos2.down())) {
				...
				if (r >= i || optionalInt.isPresent() && r >= optionalInt.getAsInt()) {
					...
					list.forEach((treeNode) -> {
						config.foliagePlacer.generate(world, random, config, Aux, treeNode, j, l, leavesPositions, box);
					});
					return true;
				} else {
					return false;
